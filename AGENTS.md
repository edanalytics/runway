...
**Retries and runs.** Within a run, the first report of a group wins: a re-send is accepted and ignored, even with different suggestions, and nothing stored is rewritten. So if the Executor ever re-queried IDRS on retry instead of re-sending what it had, the new matches would be dropped silently. Re-sent content isn't compared because any comparison would depend on how the Executor normalizes the details it hashes, and an order-sensitive one would fail runs whenever IDRS broke score ties differently. New evidence arrives as a new run with its own result rows. A restarted job's new run can also deliver groups an earlier, failed run never reported, so one job's input rows may name different source runs.

Each request is one transaction, for atomicity: a failure between the three inserts would otherwise leave a result with no suggestions, which looks like a genuine no-match and which a retry cannot repair. No row lock is taken: a run's batches arrive in sequence, and the unique constraints keep overlapping runs consistent.
