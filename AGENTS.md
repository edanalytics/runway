...
| 404 | No such run |
| 409 | A later run tried to establish input details the job's first run never reported |
...
What the app does check is structural, and independent of any normalization: **a job's input details are established by exactly one run.** The Executor extracts them on a job's first run; later runs re-report the same groups and add their own results. A later run reporting a correlation id the first run never established is rejected with a 409 and nothing is written, since that is either a fresh extraction — which belongs to a new job — or a bug that would attach one run's matches to another run's observables.
